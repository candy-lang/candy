...
type BuiltinResult<'h> = Result<SuccessfulBehavior<'h>, String>;
...
impl<'h> From<SuccessfulBehavior<'h>> for BuiltinResult<'h> {
    fn from(ok: SuccessfulBehavior<'h>) -> Self {
        Ok(ok)
    }
}
...
impl<'h> TryInto<Any<'h>> for InlineObject<'h> {
    type Error = String;

    fn try_into(self) -> Result<Any<'h>, Self::Error> {
