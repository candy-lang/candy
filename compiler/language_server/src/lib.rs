...
use crate::utils::{error_into_diagnostic, module_to_url};
...
impl CandyLanguageServer {
    ...
    async fn code_module_from_url(&self, url: Url) -> Module {
        ...
        module_from_package_root_and_url(project_directory, url, ModuleKind::Code)
    }
}
...
impl LanguageServer for CandyLanguageServer {
            ...
            while let Some((module, hints)) = hints_receiver.recv().await {
                client
                    .send_notification::<HintsNotification>(HintsNotification {
                        uri: module_to_url(module).unwrap().to_string(),
                        hints,
                    })
                    .await;
            }
    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        ...
        let content = {
            ...
            open_modules.extend(db.get_open_modules());
            text.into_bytes()
        };
