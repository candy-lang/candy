...
const candy_value_t *candy_builtin_if_else(candy_value_t *condition, candy_value_t *then, candy_value_t *otherwise)
{
    ...
    candy_value_t **capture = (body->value).function.captures;
    return function(NULL, capture);
}
...
candy_value_t *candy_builtin_list_get(candy_value_t *list, candy_value_t *idx)
{
    int index = idx->value.integer;
    return list->value.list[index];
}

candy_value_t *candy_builtin_get_argument_count(candy_value_t *function)
{
    return make_candy_int(0);
}

candy_value_t *candy_builtin_to_debug_text(candy_value_t *value)
{
    return make_candy_text("<todo>");
}

candy_value_t *candy_builtin_text_concatenate(candy_value_t *first, candy_value_t *second)
{
    char *t1 = first->value.text;
    char *t2 = second->value.text;
    strcat(t2, t1);
    return make_candy_text(t1);
}
...
const candy_value_t *candy_builtin_print(candy_value_t *value)
{
    ...
    printf("\n");
    return make_candy_tag("Nothing", NULL);
}
